...
export class LoginEmailComponent implements OnInit {
  ...
  async onFormSubmit() {
    ...
    try {
      this.isLogInFormType &&
        (await this.auth.signInWithEmailAndPassword(logInEmail, logInPassword));

      this.isSignUpFormType &&
        (await this.auth.createUserWithEmailAndPassword(
          signUpEmail,
          signUpPassword
        ));

      this.isResetFormType &&
        (await this.auth.sendPasswordResetEmail(resetEmail),
        (this.serverMessage = "Check Your Email To Reset Credentials"));
    } catch (error) {
  ...
  get passwordMatches() {
    if (this.formType === "sign-up") {
      ...
      return (
        this.signUpPassword?.value === this.signUpPasswordConfirmation?.value
      );
    }
  }
}
